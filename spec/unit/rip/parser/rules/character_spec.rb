...
RSpec.describe Rip::Parser::Rules::Character do
  ...
  describe '#character' do
    ...
    it { should parse('`\n').as(location: '`', character: { escape_special: 'n' }) }
  end

  describe '#escape_sequence' do
    subject { parser.escape_sequence }

    it { should parse('\u1234').as(escape_unicode: '1234') }

    described_class::SPECIAL_ESCAPES.each do |_, sequence|
      it { should parse("\\#{sequence}").as(escape_special: sequence) }
    end

    it { should parse('\w').as(escape_any: 'w') }
    it { should_not parse('\ ') }
  end
end
